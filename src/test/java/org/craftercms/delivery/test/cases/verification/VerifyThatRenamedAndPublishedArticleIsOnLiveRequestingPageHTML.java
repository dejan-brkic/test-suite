...
public class VerifyThatRenamedAndPublishedArticleIsOnLiveRequestingPageHTML extends DeliveryBaseTest {
	...
	public void verifyThatRenamedAndPublishedArticleIsOnLiveRequestingPageHTML() {
		...
		Assert.assertTrue(this.getWebDriverManager().getDriver().getCurrentUrl()
				.equalsIgnoreCase(getWebDriverManager().environmentProperties.getProperty("delivery.base.url") + "articles/2016/12/bar.html"));
	}

	@Parameters({"testId"})
	@AfterMethod(alwaysRun = true)
	public void afterTest(String testId) {
		if(!testId.equalsIgnoreCase("content044")) {
