 ...
 *
 */
// Test Case Studio- Site Content ID:45
public class FileRenameAgainAndDeletePageRenameTest extends StudioBaseTest {
	...
	private static Logger logger = LogManager.getLogger(FileRenameAgainAndDeletePageRenameTest.class);
	...
	public void checkDisplayedItemsBeforeDelete(String itemIconClass, String itemURL, String itemName) {
		if ((itemIconClass.contains("deleted")) || (itemIconClass.contains("fa-file-o"))) {
			switch (itemName) {
			case "bar.xml":
				Assert.assertTrue(itemURL.equalsIgnoreCase("/articles/2016/12/bar"));
				break;
			case "foo.xml":
				Assert.assertTrue(itemURL.equalsIgnoreCase("/articles/2016/12/foo"));
				break;
			default:
				Assert.assertTrue(itemURL.equalsIgnoreCase("/articles/2016/12/baz"));
				break;
			}
		}
	}
	...
	public void step16(String siteId) {
		...
		if (!(this.getWebDriverManager().waitUntilElementIsPresent("xpath", siteDropdownListElementXPath).getAttribute("class")
				.contains("site-dropdown-open")))
			this.getWebDriverManager().driverWaitUntilElementIsPresentAndDisplayed("xpath", siteContentXpath).click();
